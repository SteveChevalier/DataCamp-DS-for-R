---
title: "13_Data Visualization with ggplot2 (Part 2)"
...
# get iris dataset
#  load.Rdata("E:/One Drive/OneDrive/data_mining/DataCamp examples/iris.RData", iris)
```
...
```{r}
# get data used in this
#install.packages("carData"); ls("package:carData")
library(carData) # used later for Vocab dataset
library(MASS) # used in video example and
ls("package:MASS"); glimpse(farms)
```
...
# "10rRd" is an RColorBrewer sequential pallet, see 
  # https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/colorPaletteCheatsheet.pdf
```
Note;  "10rRd" is an RColorBrewer sequential pallet, see; 
  https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/colorPaletteCheatsheet.pdf
...
A plot showing jittered points is already provided and stored as p.
...
```
Remember, typically we'd draw our models on top of the dots, but in this case we didn't so that we could just keep recycling the p object. 
...
In this exercise we'll consider the preparations. That means we'll make sure the data is in the right format and that all the positions that we might use in our plots are defined. 
...
wt.cyl.am
```
Although you can set position using e.g. position = "dodge", defining objects promotes consistency between layers. 
...
Now it's time to use these functions! 
...
Zomming in 
  scale_x_continuous(limites = ...) or scale_y.....
  xlim()
...
```
 Always pay attention to the ggplot2 warning messages in case data has been removed from your plot! 
...
```{r}
# get mamsleep by dput(mamsleep), copy to empty R doc, mamsleep <- (copied data)
#    or get back saved from postgres 
head(mamsleep)
# Basic scatter plot
...
All non-data ink, Visual elements not part of the data, 
    (text; element_text etc.., line, rectangle; element_rect)
...
```
Background colours can dramatically change your plots' visual impact and help you develop a consistent visual identity. 
...
Removing excess lines is a great first step in reducing non-data ink. 
...
```
Apart from the salmon pink, this is a pretty nice plot! Make sure all your text is legible for the context in which your plot will be viewed. 
...
```
Which version do you like best? The one where legend.position = "bottom" is pretty nice! 
...
```{r}
# load package of themes
# install.packages("ggthemes") # done
library(ggthemes)
# example use  z + theme_tufte() # remove all non-data ink

# theme_update() # add to a theme
# theme_set(your theme) set it back theme_set(original)
```
Updating Themes
...
Start with scatter plot and view all points
...
m + 
  geom_errorbar(ymin = wt.avg - sd, ymax = wt.avg + sd) +
  geom_errorbar(aes(ymin = wt.avg - sd, ymax = wt.avg + sd), width = 0.1)
```


Best Practices: Pie Charts (take away, don't use pies, use paralle bar charts)
...
```{r}
# scraped (copy/paste) from dcamp lesson, fixed in libre calc, excel and notepad++ (year seperated from variety)
barley <- read.csv("E:/One Drive/OneDrive/data_mining/DataCamp examples/barley.csv")
names(barley) <- c("yield","variety","year","site" )
barley
...
ggplot(barley, 
  aes(x = year, y = yield, col = site, group = site, fill = site)) +
...
**Case Study**
California Health Information Survey (CHIS) - Descriptive Statistics

Limited for DataCamp by 2009 adult
RBMI, BMI_P, RACEHPR2, SRSEX, SRAGE_P, MARIT2, AB1, ASTCUR, AB51, POVLL

"BMI Category Description", "Race", "Sex", "Age", "Marital Status", "General Health Condition", "Current Asthma Status", "Type I or Type II Diabetes", "Poverty level"

filter(latino, asian, african america, white)

age
ggplot(adult, aes(x = SRAGE_P)) + geom_histogram()
diff(range(adult@SRAGE_P)) / 30 # default bin width, = 2.2333

BMI wt / hieght squared, peaks at certain ages, bin stat smal
ggplot(adult, aes(x = BMI_P)) + geom_histogram()
skewed to the right, ?outliers

compare BMI vs Age. continues and ordinal
Range         Clasification
0 - 18.49     Underweight
...
Histogram age vs bmi
ggplot(adult, aes ( x = SRAGE_P, fill = factor(RBMI))) + geom_histogram()

ggplot(adult, aes ( x = SRAGE_P, fill = factor(RBMI))) +
  geom_histogram(aes(y= ..count../sum(..count..))).
      binwidth = 1, position = "fill") + .... to be continued in lessons
      
load data set from source (stata version was only one that worked), 
  subset to just col and rows required for this datacamp lesson
```{r}
# input Stata file
library(foreign)
adult_strat_in <- read.dta("E:/One Drive/OneDrive/data_mining/DataCamp examples/CHIS/chis09_adult_stata/chis09_adult_stata/ADULT.dta") 
# create final dataset selected columns and filtered rows
adult_subset_stata <- adult_strat_in %>% 
    select(rbmi, bmi_p, racehpr2, srsex, srage_p, marit2,ab1,astcur,ab51,povll) %>%
    filter(racehpr2 %in% c("WHITE","LATINO","ASIAN","AFRICAN AMERICAN"))
# View(adult_subset_stata)
```

Save to postgres schema 
```{r}
# write both datasets to postgres schema
library(RPostgreSQL);library(dplyr);library(DBI)
con_postgres <- dbConnect(PostgreSQL(), 
                         dbname = "ca_health_info_survey",  host = "127.0.0.1", 
                         port = 5432, user = "postgres", password = "admin1")
# get list of tables
dbListTables(con_postgres)

# create / write data to, a table
dbWriteTable(con_postgres, "adult_strat_in", adult_strat_in, append = TRUE)
dbWriteTable(con_postgres, "adult_subset_stata", adult_subset_stata, append = TRUE)
dbWriteTable(con_postgres, "adult", adult_subset_stata, append = TRUE)
...
```{r}
# create dataset that matches name used in class
# adult <- adult_subset_stata
# saved in postgres use this to get back
sql_stmt <- "select * from public.adult"  
adult <- dbGetQuery(con_postgres, sql_stmt)
# View(adult)
```
...
There is an unexpectedly large number of very old people. it looks like everyone 85 and above has been categorized as 85 years old. 

Default Binwidths

If you don't specify the binwidth argument inside geom_histogram() you can tell from the message that 30 bins are used by default. This will then specify the binwidth that is used. What is this binwidth for the age variable, SRAGE_P, of the adult dataset? = 2.33, = diff(range(adult@SRAGE_P)) / 30 
...
# save this work to postgres
dbWriteTable(con_postgres, "adult_cleaned", adult, append = TRUE)

# these two fail DONT RUN
# Relabel the race variable
# adult$racehpr2 <- factor(adult$racehpr2, labels = c("Latino", "Asian", "African American", "White"))
# Relabel the BMI categories variable
# adult$rbmi <- factor(adult$rbmi, labels = c("Under-weight", "Normal-weight", "Over-weight", "Obese"))
```
 Cleaning data is an important part of your job as a data scientist. 
...
```
Mosaic plots are one of my favourite plot types - we don't see them nearly enough! Let's get more detailed in the next chapter. 
...
You've made a mosaic plot from scratch - good work! This became a hyper-informative plot with the added statistics - but we still need proper labels. 
...
With some minor tweaking we could easily turn this into a publication-quality plot. 
...
```
These are intuitive and easy-to-generate custom plots! This concludes the end of the second ggplot2 course. 

---
title: '17_Exploratory Data Analysis in R a Case Study'
...
```{r}
# to get from DB -----------------
library(RPostgreSQL);library(DBI);library(dplyr)
# # connect to postgres database
con_postgres <- dbConnect(PostgreSQL(), dbname = "d_camp_data",
      host = "127.0.0.1", port = 5432,user = "postgres", password = "admin1")
dbListTables(con_postgres)

votes <- dbGetQuery(con_postgres, "select * from public.votes;")
  print("votes");glimpse(votes)
states <- dbGetQuery(con_postgres, "select * from public.states;")
  print("states");glimpse(states)
resolutions <- dbGetQuery(con_postgres, "select * from public.resolutions;")
  print("resolutions");glimpse(resolutions)
descriptions <- dbGetQuery(con_postgres, "select * from public.descriptions")
  print("descriptions");glimpse(descriptions)
  
```

```{r}
# to get from csv files
# votes from kaggle (see also states)
  # path <- "E:/One Drive/OneDrive/data_mining/d_UN_voting_histor_general_assembly/"
  # fileName <- "votes.csv"; pathAndFile <- paste(path, fileName, sep = "")
  # votes <- read.csv(pathAndFile)
  # print("votes");glimpse(votes)
  # 
  # fileName <- "states.csv"; pathAndFile <- paste(path, fileName, sep = "")
  # states <- read.csv(pathAndFile)
  #   print("states");glimpse(states)
  # 
  # fileName <- "resolutions.csv"; pathAndFile <- paste(path, fileName, sep = "")
  # resolutions <- read.csv(pathAndFile)
  #   print("resolutions");glimpse(resolutions)
  #
  # fileName <- "E:/One Drive/OneDrive/data_mining/d_UN_voting_histor_general_assembly/descriptions.rds"
  # descriptions <- readRDS(fileName, refhook = NULL)

```

```{r}
# optionaly write to postgres data base, must set connection first (done)
# connect to postgres database
  # con_postgres <- dbConnect(PostgreSQL(), dbname = "d_camp_data",
  #       host = "127.0.0.1", port = 5432,user = "postgres", password = "admin1")
  # dbWriteTable(con_postgres, "votes", votes, append = TRUE)
  # dbWriteTable(con_postgres, "states", states, append = TRUE)
  # dbWriteTable(con_postgres, "resolutions", resolutions, append = TRUE)
  # dbWriteTable(con_postgres, "descriptions", descriptions, append = TRUE)
...
```{r}
# class had a step here to add country but it exists in kaggle version of votes
# create votes_processed as per the class
...
summarize(votes_processed, total = n(), percent_yes = mean(vote == 1))
```
...
by_year_country <-
  ...
  summarize(total = n(),percent_yes = mean(vote == 1)) %>%
  select(country, year, total, percent_yes)
...
states_selected <- c("United States of America", "United Kingdom", "France", "India")
# Filter by_year_country: filtered_4_countries
filtered_4_countries <- 
  filter(by_year_country, country %in% states_selected)
...
countries <- c("United States of America", "United Kingdom",
               "France", "Japan", "Brazil", "India")

# Filtered by_year_country: filtered_6_countries
filtered_6_countries <- 
  filter(by_year_country, country %in% countries)
...
```{r}
# Add three more countries to this list (to see country list "View" by_country ---->>>>)
countries <- c("United States of America", "United Kingdom",
               "France", "Japan", "Brazil", "India",
...
# model <- lm(percent_yes ~ year, data = afghanistan)  ~ = "explained by"
```{r}
...
print(bind_rows(US_tidied,UK_tidied))
```
...
```
Notice that there are now only 61 countries (in DataCamp class data) with significant trends. 
...
resolutions # same data as descriptions table from data camp, many more rows though
# CHANGE LOGIC TO USE THIS IN FINAL STUDY
```
...
print(votes_processed)
# Print the descriptions dataset -- change to "resolutions"
print(descriptions)
# Join them together based on the "rcid" and "session" columns
votes_joined <- inner_join(votes_processed, descriptions, by = c("rcid", "assembly_session"))
```
...
country_topic_filtered <- country_topic_coefficients %>%
    filter(term == "year") %>%
    mutate(p.adjusted = p.adjust(p.value)) %>%
    filter(term == "year", p.adjusted < .05)
...
--------------- IDEAS for book chapter ---------
sum votes by continent, compare votes from each continent
add resolution description to last few US voted no on?  something like that

what countries agree
what countries don't agree
who does US agree with the most / least
  same by continient
  same by topic
